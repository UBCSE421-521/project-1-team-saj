...
void thread_tick(void)
{
  ...
  //This is the implementation for mlfqs which calculates recent_cpu and load avg based on ticks interval
  if (thread_mlfqs)
  {
    t->recent_cpu = get_sum_fp_int(t->recent_cpu, 1);
...
void Donate_Prioirty(struct thread *t)
{
  enum intr_level old_level = intr_disable();
  list_sort(&t->acquired_locks, sort_acquired_locks, NULL);
  t->priority = list_entry(list_front(&t->acquired_locks), struct lock, elem)->max_priority;
...
void Calculate_Load_avg(void)
{
  ...
  load_avg = get_sum_fp(get_div_fp_int(get_product_fp_int(load_avg, 59), 60), get_div_fp_int(conv_int_to_fix(ready_threads), 60));
}

/* Updates the recent cpu value */
void Update_Recent_Cpu(struct thread *t, void *aux UNUSED)
{
  int coefficient = get_div_fp(get_product_fp_int(2, load_avg),
                               get_sum_fp_int(get_product_fp_int(2, load_avg), 1));
  t->recent_cpu = get_sum_fp(get_product_fp(coefficient, t->recent_cpu), conv_int_to_fix(t->nice));
}

void Update_Priority(struct thread *t, void *aux UNUSED)
{
  int new_priority = PRI_MAX - conv_fix_to_int_nearest(get_div_fp_int(t->recent_cpu, 4)) - (t->nice * 2);
  ...
  // We could maybe implement preemption here.
}
...
{
  return conv_fix_to_int_nearest(get_product_fp_int(load_avg, 100));
}

/* Returns 100 times the current thread's recent_cpu value. */
int thread_get_recent_cpu(void)
{
  return conv_fix_to_int_nearest(get_product_fp_int(thread_current()->recent_cpu, 100));
}
